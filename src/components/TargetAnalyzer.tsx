...
import { X, Crosshair, RotateCcw, Ruler, CheckCircle, Info, RefreshCw } from 'lucide-react';
import { Button } from '../components/ui/button';

interface Point { x: number; y: number; }

export interface GroupAnalysis {
  shotCount: number; groupSize: number; groupSizeMoa: number | null;
  meanRadius: number; meanRadiusMoa: number | null;
  width: number; height: number; distance: string;
}

interface TargetAnalyzerProps {
  imageData: string; groupId: string; distance?: string;
...
export function TargetAnalyzer({ imageData, groupId, distance = '', onSave, onClose }: TargetAnalyzerProps) {
  ...
  const [disabledShots, setDisabledShots] = useState<Set<number>>(new Set());
  ...
  const activeShots = shots.filter((_, i) => !disabledShots.has(i));
  ...
  const draw = useCallback(() => {
    ...
    shots.forEach((pt, i) => {
      const cp = toC(pt);
      const disabled = disabledShots.has(i);
      ctx.globalAlpha = disabled ? 0.3 : 1;
      ctx.beginPath(); ctx.arc(cp.x, cp.y, markerR, 0, Math.PI * 2);
      ctx.strokeStyle = disabled ? '#888' : '#39ff14'; ctx.lineWidth = disabled ? 1 : 1.5; ctx.stroke();
      ctx.fillStyle = disabled ? '#888' : '#39ff14';
    ...
    const activePts = activeShots.map(p => ({ x: p.x * W, y: p.y * H }));
    if (analysis && activePts.length >= 2) {
      const cx = activePts.reduce((s, p) => s + p.x, 0) / activePts.length;
      const cy = activePts.reduce((s, p) => s + p.y, 0) / activePts.length;
      activePts.forEach(cp => {
  ...
  }, [imgLoaded, canvasSize, scalePt1, scalePt2, shots, disabledShots, analysis, activeShots, calcPpu]);

  useEffect(() => { draw(); }, [draw]);

  const computeAnalysisFor = useCallback((pts_in: Point[]) => {
    const ppu = calcPpu(canvasSize.w, canvasSize.h);
    if (!ppu || pts_in.length < 2) return;
    const pts = pts_in.map(p => ({ x: p.x * canvasSize.w, y: p.y * canvasSize.h }));
  ...
  }, [calcPpu, canvasSize, sessionDistance]);

  // Re-compute live when shots toggled in results step
  useEffect(() => {
    if (step === 'results') {
      const active = shots.filter((_, i) => !disabledShots.has(i));
      if (active.length >= 2) computeAnalysisFor(active);
    }
  }, [disabledShots, step, shots, computeAnalysisFor]);

  const computeAnalysis = () => {
    const active = shots.filter((_, i) => !disabledShots.has(i));
    computeAnalysisFor(active);
    setStep('results');
    setOverlayPos({ x: 16, y: 16 });
  };

  const handleRestart = () => {
    setStep('scale1'); setScalePt1(null); setScalePt2(null);
    setShots([]); setDisabledShots(new Set()); setAnalysis(null);
  };

  const toggleShot = (i: number) => {
    setDisabledShots(prev => { const n = new Set(prev); if (n.has(i)) n.delete(i); else n.add(i); return n; });
  };
  ...
  const handleMouseMove = (e: React.MouseEvent) => { if (!dragging) return; setOverlayPos({ x: e.clientX - dragOffset.x, y: e.clientY - dragOffset.y }); };
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const pt: Point = { x: (e.clientX - rect.left) / rect.width, y: (e.clientY - rect.top) / rect.height };
  ...
  const drawOverlayOnCanvas = (ctx: CanvasRenderingContext2D, a: GroupAnalysis, x: number, y: number) => {
    ...
    const fontSize = 13, lineH = fontSize * 1.6, padX = 10, padY = 8;
    ...
    ctx.fillStyle = '#39ff14'; ctx.textAlign = 'left'; ctx.textBaseline = 'top';
    lines.forEach((line, i) => {
      ctx.font = `${i === 0 && !!a.distance ? '700' : '600'} ${fontSize}px "JetBrains Mono", monospace`;
  ...
  const handleSave = () => {
    ...
    if (ctx) {
      ...
      drawOverlayOnCanvas(ctx, analysis, Math.max(0, overlayPos.x - canvasX) * scaleX, Math.max(0, overlayPos.y - canvasY) * scaleY);
    }
    onSave(analysis, canvas.toDataURL('image/jpeg', 0.92));
  };

  const stepLabel: Record<Step, string> = {
    scale1: '1 — Click first scale point', scale2: '2 — Click second scale point',
    shots: '3 — Click each shot hole', results: '4 — Review & save',
  };
    ...
    <div className="fixed inset-0 z-50 flex flex-col" style={{ backgroundColor: '#0a0a0a' }}>
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-amber-400 font-mono px-2 py-0.5 rounded border border-amber-900/50 bg-amber-900/10">{stepLabel[step]}</span>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={handleRestart} className="flex items-center gap-1.5 text-xs text-slate-500 hover:text-amber-400 transition-colors px-2 py-1 rounded border border-slate-700 hover:border-amber-600">
            <RefreshCw className="w-3.5 h-3.5" /> Restart
          </button>
          <button onClick={onClose} className="text-slate-500 hover:text-white transition-colors ml-1"><X className="w-5 h-5" /></button>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 flex items-center justify-center p-4 overflow-auto relative" onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
          ...
          {analysis && step === 'results' && (
            <div onMouseDown={handleOverlayMouseDown} style={{
              position: 'absolute', left: overlayPos.x, top: overlayPos.y,
              cursor: dragging ? 'grabbing' : 'grab', userSelect: 'none', zIndex: 20,
              backgroundColor: 'rgba(0,0,0,0.65)', border: '1px solid #39ff14',
              boxShadow: '0 0 10px rgba(57,255,20,0.25)', borderRadius: 6,
              padding: '8px 12px', fontFamily: '"JetBrains Mono", monospace',
              fontSize: 13, color: '#39ff14', lineHeight: 1.65, minWidth: 160,
            }}>
        ...
        {/* Right panel */}
        <div className="w-72 flex-shrink-0 border-l border-slate-800 flex flex-col overflow-y-auto" style={{ backgroundColor: '#0d0d0d' }}>

          {/* Instructions */}
          <div className="p-4 border-b border-slate-800">
            <div className="flex items-center gap-2 mb-2.5">
              <Info className="w-3.5 h-3.5 text-amber-400 flex-shrink-0" />
              <span className="text-xs font-bold text-amber-400 uppercase tracking-widest">How to use</span>
            </div>
            <ol className="space-y-2">
              {[
                { n: '1', text: 'Enter shooting distance below.' },
                { n: '2', text: 'Set reference length (e.g. 1"). Click two points on the target that are exactly that distance apart.' },
                { n: '3', text: 'Click each bullet hole to place a numbered marker.' },
                { n: '4', text: 'Hit Compute Results. Tap any shot number to exclude it from the analysis.' },
                { n: '5', text: 'Drag the green results overlay to reposition it, then Save.' },
              ].map(({ n, text }) => (
                <li key={n} className="flex gap-2">
                  <span className="flex-shrink-0 w-4 h-4 rounded-full bg-amber-900/40 border border-amber-700/50 text-amber-400 text-[10px] font-bold flex items-center justify-center mt-0.5">{n}</span>
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-3 text-xs items-center">
                ...
                {scalePt1 && (
                  <button onClick={() => { setScalePt1(null); setScalePt2(null); setStep('scale1'); }}
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-400 uppercase tracking-widest">
                    Shots ({activeShots.length}{disabledShots.size > 0 ? `/${shots.length}` : ''})
                  </span>
                </div>
              ...
              {step === 'shots' && shots.length === 0 && <p className="text-xs text-slate-500">Click each bullet hole on the image.</p>}
              {shots.length > 0 && (
                <>
                  <div className="flex flex-wrap gap-1.5">
                    {shots.map((_, i) => {
                      const off = disabledShots.has(i);
                      return (
                        <button key={i} onClick={() => toggleShot(i)}
                          title={off ? 'Click to include' : 'Click to exclude'}
                          className={`border text-xs font-mono transition-all flex items-center justify-center ${off ? 'bg-slate-900 border-slate-700 text-slate-600 opacity-40' : 'bg-green-900/30 border-green-700/50 text-green-400'}`}
                  ...
                  {disabledShots.size > 0 && <p className="text-xs text-slate-600">Dimmed shots are excluded from analysis.</p>}
                </>
              )}
            </div>
            ...
            <div className="space-y-2 pt-2 border-t border-slate-800">
              {step === 'shots' && activeShots.length >= 2 && (
              ...
              {step === 'shots' && activeShots.length < 2 && <p className="text-xs text-slate-600 text-center">Mark at least 2 shot holes</p>}
              {shots.length > 0 && step !== 'results' && (
                <button onClick={() => { setShots([]); setDisabledShots(new Set()); }} className="w-full text-xs text-slate-500 hover:text-red-400 transition-colors py-1">Clear all shots</button>
              )}
            </div>
